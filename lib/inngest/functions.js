...
export const AiResumeAgent = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const uploadFileUrl = await step.run("upload-to-imagekit", async () => {
      ...
      // UPSERT ensures the record exists for the frontend to poll
      await db.historyTable.upsert({
        ...
        create: {
          ...
          aiAgentType,
          userEmail,
          content: null // Keep null until AI finishes
        }
      });
    ...
    const aiResponse = await step.run("ai-analysis", async () => {
      ...
      return report.output?.[0]?.content || report.output;
    });
    ...
    await step.run("save-final-report", async () => {
      ...
      return await db.historyTable.update({
        where: { recordId: recordId },
