...
const DATA = 
  ...
  },
  {
    "id": 8,
    "name": "Tables",
    "desc": "Represent information in structured format.",
    "exercises": [
  ...
  },
  {
    "id": 11,
    "name": "Audio & Video",
    "desc": "Add multimedia components for rich experiences.",
    "exercises": [
      ...
      {"name": "Add Subtitles", "slug": "add-titles", "xp": 40, "difficulty": "medium"},
...
export async function GET(req) {
  try {
    // We use Promise.all with map instead of forEach to ensure 
    // all async operations complete before sending the response.
    await Promise.all(
      DATA.map(async (item) => {
        return db.courseChapter.create({
          data: {
            courseId: 2,           // Static ID as seen in your screenshot
            desc: item?.desc,      // Mapping description
            exercises: item?.exercises, // Mapping JSON exercises
            name: item?.name, 
            chapterId:item?.id     // Mapping chapter name
            // Note: Prisma handles 'id' automatically based on your schema.
            // If you want to store your JSON 'id' elsewhere, you'd need a 
            // dedicated field for it in the schema.
          },
        });
      })
    );
